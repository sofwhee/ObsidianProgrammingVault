...
If you're working on the same project with someone else, that's a lot of merge conflicts and breaking of things...
...
DraggableClothing had dependencies on Character, UIRaycast.

Instead, DraggableClothing could depend on an Interface (IDrag) to connect it to Game Manager.

IBeginDrag -> Game Manager looks at mouse position. Raycast. If mouse is over a draggable clothing item, make a clone and make it follow the mouse.

IEndDrag -> Game Manager looks at mouse position. Raycast. IF mous is over character.

Now DraggableClothing has one dependency.